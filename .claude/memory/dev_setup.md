...
---
User runs the iOS app on the **iPhone 17 Pro simulator** locally — that's the default device for `pnpm mobile:run`.
...
**How to apply:** When the user asks to run / test / build the iOS app, default to the relevant `pnpm mobile:*` script. If the iPhone 17 Pro simulator is ever decommissioned (Xcode upgrade, etc.) update the `mobile:run` script in `package.json` rather than telling the user to switch devices manually.
